...
import com.dmeyc.dmestoreyfm.bean.NOpkScocktRecordBean;
...
public class BClientSorckerInActivity extends BaseActivity {
    ...
    NOpkScocktRecordBean sockInBean;
...
//        }else {
//            getpkrecord();
//            url=  Constant.API.YFM_GETNOPKRECORD;
//        }
...
//                et_sokinteamone.setText(sockInBean.getData().get(clickitem).getTeam_a_score()+"");
                et_sokinteamone.setHint(sockInBean.getData().get(clickitem).getTeam_a_score()+"");
//                et_teamtwosokin.setText(sockInBean.getData().get(clickitem).getTeam_b_score()+"");
                et_teamtwosokin.setHint(sockInBean.getData().get(clickitem).getTeam_b_score()+"");
...
public int clickitem=-1;
    ...
    public class SockInAdapter extends BaseAdapter{
        @Override
        public int getCount() {
            return sockInBean.getData().size();
        }
        @Override
        public Object getItem(int i) {
            return null;
        ...
        public View getView(final int i, View view, ViewGroup viewGroup) {
            ...
            GlideUtil.loadImage(BClientSorckerInActivity.this,sockInBean.getData().get(i).getTeam_a_member_a_logo(),myViewHolder.cv_actionone);
            GlideUtil.loadImage(BClientSorckerInActivity.this,sockInBean.getData().get(i).getTeam_a_member_b_logo(),myViewHolder.cv_actiontwo);
            GlideUtil.loadImage(BClientSorckerInActivity.this,sockInBean.getData().get(i).getTeam_b_member_a_logo(),myViewHolder.cv_actionthree);
            GlideUtil.loadImage(BClientSorckerInActivity.this,sockInBean.getData().get(i).getTeam_b_member_b_logo(),myViewHolder.cv_actionfour);
           myViewHolder.action_onename.setText(sockInBean.getData().get(i).getTeam_a_member_a_nickname());
           myViewHolder.action_twoname.setText(sockInBean.getData().get(i).getTeam_a_member_b_nickname());
            myViewHolder.tv_actonthreename.setText(sockInBean.getData().get(i).getTeam_b_member_a_nickname());
            myViewHolder.tv_action_fourname.setText(sockInBean.getData().get(i).getTeam_b_member_b_nickname());
            myViewHolder.tv_teamsocke.setText(sockInBean.getData().get(i).getTeam_a_score()+"");
            myViewHolder.tv_teambsocker.setText(sockInBean.getData().get(i).getTeam_b_score()+"");
            myViewHolder.ll_sockclick.setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View view) {
//                                ToastUtil.show(i+"ssssss");
...
public void ToSockInNoPk(){
    ...
    RestClient.getYfmNovate(this).post(Constant.API.YFM_TOSOCKIN, new ParamMap.Build()
            ...
            .addParams("activity_id",getIntent().getIntExtra("activityid",-1))
                    ...
                    .addParams("group_match_id",sockInBean.getData().get(clickitem).getId())
...
public void getnoPKrecord(){
    RestClient.getYfmNovate(this).post(Constant.API.YFM_GETNOPKRECORD, new ParamMap.Build()
            .addParams("user_token", SPUtils.getStringData(Constant.Config.TOKEN))
            .addParams("activity_id",getIntent().getIntExtra("activityid",-1))
            .build(), new DmeycBaseSubscriber<NOpkScocktRecordBean>() {
        @Override
        public void onSuccess(final NOpkScocktRecordBean bean) {
//            ToastUtil.show(bean.getMsg());
