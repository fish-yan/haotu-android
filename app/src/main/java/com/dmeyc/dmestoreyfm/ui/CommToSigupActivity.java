...
public class CommToSigupActivity extends BaseActivity {
...
//    ImageView iv_setsex1;
    ...
    protected void initData() {
        registerReceiver(new CommToSigupActivity.WXBroadcastReceiver(),
...
//                    SPUtils.savaStringData(Constant.Config.ORDER_ID,orderId.contains(",") ? orderId.split(",")[0] : orderId);
        ...
        IWXAPI msgApi = WXAPIFactory.createWXAPI(CommToSigupActivity.this, null);
...
//                finish();
                ...
                if(istruename){
                    startActivity(new Intent(CommToSigupActivity.this, MainActivity.class));
                    finish();
                }else {
                    goShop();
                }
    ...
    class iconAdapter extends RecyclerView.Adapter<iconAdapter.ViewHolder>{
        ...
        @Override
        public CommToSigupActivity.iconAdapter.ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {

            View view =getLayoutInflater().inflate(R.layout.adapter_settitlenumb,null);

            return new CommToSigupActivity.iconAdapter.ViewHolder(view);
        }

        int clickpos=-1;
        ArrayList<TextView> tvs=new ArrayList<>();
        @Override
        public void onBindViewHolder(final CommToSigupActivity.iconAdapter.ViewHolder holder, final int position) {
//            GlideUtil.loadImage(SubmitOrderActivity.this,activityDeatilBean.getData().getSign_up_list().get(position).getUrl(),holder.civ_avatar);
            ...
            if(-1==clickpos) {
                tvs.get(0).setBackground(CommToSigupActivity.this.getResources().getDrawable(R.drawable.shap_settitlebg));
            }else {
...
//                        ToastUtil.show(position+"111");
                        if(ii==position){
                            tvs.get(position).setBackground(CommToSigupActivity.this.getResources().getDrawable(R.drawable.shap_settitlebg));
                        }else {
                            tvs.get(ii).setBackgroundColor(CommToSigupActivity.this.getResources().getColor(R.color.gb));
                        }
                    ...
                    for (int i=0;i<poss.size();i++){
                        if(position==i){
                            ...
                            if(TextUtils.isEmpty(seatBean.getData().getMember_list().get(i*2).getUser_nickname())){
                                iv_set1.setBackground(CommToSigupActivity.this.getDrawable(R.drawable.icon_seat));
                            }else {
                                iv_set1.setBackground(CommToSigupActivity.this.getDrawable(R.drawable.set_ku));
                            }
...
//                tv_titl=  itemView.findViewById(R.id.tv_titl);
    ...
    public class  payAdapter extends BaseAdapter {
        ...
        CommToSigupActivity.viewHoder viewHoder;
        @Override
        public View getView(final int i, View view, ViewGroup viewGroup) {
            if(view==null){
                view=getLayoutInflater().inflate(R.layout.adapter_payitem,null);
                viewHoder=new CommToSigupActivity.viewHoder(view);
                view.setTag(viewHoder);
            }else {
                viewHoder=(CommToSigupActivity.viewHoder)view.getTag();
            }
//                rbs.add(viewHoder.rb_alipay);
            ...
            if((-1==clickpos&&0==i)||i==clickpos){
                viewHoder.rb_alipay.setBackground(CommToSigupActivity.this.getResources().getDrawable(R.drawable.pay_check));
            }else {
                viewHoder.rb_alipay.setBackground(CommToSigupActivity.this.getResources().getDrawable(R.drawable.pay_unchke));
            }
            return view;
        }
...
//            return 10;
        ...
        public View getView(final int i, View view, ViewGroup viewGroup) {
            ...
            if(clickgrpos==i){
                if(clicktype==0){
                    GlideUtil.loadImage(CommToSigupActivity.this, SPUtils.getStringData(Constant.Config.AVATAR),holders.iv_set1);
                    Glide.with(CommToSigupActivity.this).load(seatBean.getData().getMember_list().get(i*2+1)).into(holders.iv_set2);
...
//                            holders.tv_set2name.setText("");
                    ...
                    Glide.with(CommToSigupActivity.this).load(seatBean.getData().getMember_list().get(i*2)).into(holders.iv_set1);
                    GlideUtil.loadImage(CommToSigupActivity.this, SPUtils.getStringData(Constant.Config.AVATAR),holders.iv_set2);
//                            holders.tv_set1name.setText("");
                ...
                if(TextUtils.isEmpty(seatBean.getData().getMember_list().get(i*2+1).getUser_nickname())){
                    holders.iv_setsex2.setVisibility(View.GONE);
                    holders.tv_set2name.setText("");
                    Glide.with(CommToSigupActivity.this).load(R.drawable.set_rabbit).into(holders.iv_set2);
                    ...
                    holders.tv_set2name.setText(seatBean.getData().getMember_list().get(i*2+1).getUser_nickname());
                    Glide.with(CommToSigupActivity.this).load(seatBean.getData().getMember_list().get(i*2+1).getUser_logo()).into(holders.iv_set2);
                }

                if(TextUtils.isEmpty(seatBean.getData().getMember_list().get(i*2).getUser_nickname())){
                    holders.iv_setsex1.setVisibility(View.GONE);
                    holders.tv_set1name.setText("");
                    Glide.with(CommToSigupActivity.this).load(R.drawable.set_rabbit).into(holders.iv_set1);
                }else {
                    holders.iv_setsex1.setVisibility(View.VISIBLE);
                    ...
                    holders.tv_set1name.setText(seatBean.getData().getMember_list().get(i*2).getUser_nickname());
                    Glide.with(CommToSigupActivity.this).load(seatBean.getData().getMember_list().get(i*2).getUser_logo()).into(holders.iv_set1);
                }
            }
    ...
    TextView tv_toinfo;
    public void goShop(){
        dialog  = new Dialog(CommToSigupActivity.this, R.style.MyDialog);
        //设置它的ContentView
...
//        alltv_price=dialog.findViewById(R.id.alltv_price);
        ...
        tv_toinfo.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                startActivity(new Intent(CommToSigupActivity.this, TrueNameActivity.class));
            }
        });
