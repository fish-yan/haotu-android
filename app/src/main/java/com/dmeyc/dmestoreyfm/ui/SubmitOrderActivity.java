...
//
//                });
                registerReceiver(new SubmitOrderActivity.WXBroadcastReceiver(),
...
//                finish();
                ...
                if(istruename){
                    startActivity(new Intent(SubmitOrderActivity.this, MainActivity.class));
                    finish();
                }else {
                    goShop();
                }
    ...
    class iconAdapter extends RecyclerView.Adapter<iconAdapter.ViewHolder>{
        ...
        @Override
        public SubmitOrderActivity.iconAdapter.ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {

            View view =getLayoutInflater().inflate(R.layout.adapter_settitlenumb,null);

            return new SubmitOrderActivity.iconAdapter.ViewHolder(view);
        }

        int clickpos=-1;
        ArrayList<TextView> tvs=new ArrayList<>();
        @Override
        public void onBindViewHolder(final SubmitOrderActivity.iconAdapter.ViewHolder holder, final int position) {
//            GlideUtil.loadImage(SubmitOrderActivity.this,activityDeatilBean.getData().getSign_up_list().get(position).getUrl(),holder.civ_avatar);
            ...
            if(-1==clickpos) {
                tvs.get(0).setBackground(SubmitOrderActivity.this.getResources().getDrawable(R.drawable.shap_settitlebg));
            }else {
...
//                        ToastUtil.show(position+"111");
                        if(ii==position){
                            tvs.get(position).setBackground(SubmitOrderActivity.this.getResources().getDrawable(R.drawable.shap_settitlebg));
                        }else {
                            tvs.get(ii).setBackgroundColor(SubmitOrderActivity.this.getResources().getColor(R.color.gb));
                        }
...
//                tv_titl=  itemView.findViewById(R.id.tv_titl);
    ...
    public class  payAdapter extends BaseAdapter {
        ...
        SubmitOrderActivity.viewHoder viewHoder;
        @Override
        public View getView(final int i, View view, ViewGroup viewGroup) {
            if(view==null){
                view=getLayoutInflater().inflate(R.layout.adapter_payitem,null);
                viewHoder=new SubmitOrderActivity.viewHoder(view);
                view.setTag(viewHoder);
            }else {
                viewHoder=(SubmitOrderActivity.viewHoder)view.getTag();
            }
//                rbs.add(viewHoder.rb_alipay);
            ...
            if((-1==clickpos&&0==i)||i==clickpos){
                viewHoder.rb_alipay.setBackground(SubmitOrderActivity.this.getResources().getDrawable(R.drawable.pay_check));
            }else {
                viewHoder.rb_alipay.setBackground(SubmitOrderActivity.this.getResources().getDrawable(R.drawable.pay_unchke));
            }
            return view;
        }
            ...
            if(TextUtils.isEmpty(seatBean.getData().getMember_list().get(i*2).getUser_nickname())){
             ...
             if(TextUtils.isEmpty(seatBean.getData().getMember_list().get(i*2+1).getUser_nickname())){
                 ...
                 holders.tv_set2name.setText(seatBean.getData().getMember_list().get(i*2+1).getUser_nickname());
                    ...
                    if(clickgrpos==i){
                        if(clicktype==0){
                            GlideUtil.loadImage(SubmitOrderActivity.this, SPUtils.getStringData(Constant.Config.AVATAR),holders.iv_set1);
                            Glide.with(SubmitOrderActivity.this).load(seatBean.getData().getMember_list().get(i*2+1)).into(holders.iv_set2);
...
//                            holders.tv_set2name.setText("");
                            ...
                            Glide.with(SubmitOrderActivity.this).load(seatBean.getData().getMember_list().get(i*2)).into(holders.iv_set1);
                            GlideUtil.loadImage(SubmitOrderActivity.this, SPUtils.getStringData(Constant.Config.AVATAR),holders.iv_set2);
//                            holders.tv_set1name.setText("");
                ...
                if(TextUtils.isEmpty(seatBean.getData().getMember_list().get(i*2+1).getUser_nickname())){
                    holders.iv_setsex2.setVisibility(View.GONE);
                    holders.tv_set2name.setText("");
                    Glide.with(SubmitOrderActivity.this).load(R.drawable.set_rabbit).into(holders.iv_set2);
                    ...
                    holders.tv_set2name.setText(seatBean.getData().getMember_list().get(i*2+1).getUser_nickname());
                    Glide.with(SubmitOrderActivity.this).load(seatBean.getData().getMember_list().get(i*2+1).getUser_logo()).into(holders.iv_set2);
                }

                        if(TextUtils.isEmpty(seatBean.getData().getMember_list().get(i*2).getUser_nickname())){
                            holders.iv_setsex1.setVisibility(View.GONE);
                            holders.tv_set1name.setText("");
                            Glide.with(SubmitOrderActivity.this).load(R.drawable.set_rabbit).into(holders.iv_set1);
                        }else {
                            holders.iv_setsex1.setVisibility(View.VISIBLE);
                            ...
                            holders.tv_set1name.setText(seatBean.getData().getMember_list().get(i*2).getUser_nickname());
                            Glide.with(SubmitOrderActivity.this).load(seatBean.getData().getMember_list().get(i*2).getUser_logo()).into(holders.iv_set1);
                        }
...
//                    alc2.clear();
    ...
    TextView tv_toinfo;
    public void goShop(){
        dialog  = new Dialog(SubmitOrderActivity.this, R.style.MyDialog);
        //设置它的ContentView
...
//        alltv_price=dialog.findViewById(R.id.alltv_price);
        ...
        tv_toinfo.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                dialog.dismiss();
                startActivity(new Intent(SubmitOrderActivity.this, TrueNameActivity.class));
            }
        });
...
//                    SPUtils.savaStringData(Constant.Config.ORDER_ID,orderId.contains(",") ? orderId.split(",")[0] : orderId);
            WXSubmitBean.DataBean.PayInfoBean data = wxSubmitBean.getData().getPayInfo();
                    ...
                    IWXAPI msgApi = WXAPIFactory.createWXAPI(SubmitOrderActivity.this, null);
