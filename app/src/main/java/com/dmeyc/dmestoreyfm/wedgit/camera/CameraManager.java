...
public class CameraManager implements ICameraHelper {
    ...
    public static final int[] RES_DRAWABLE_FLASHLIGHT = {/*R.drawable.selector_btn_flashlight_auto,*/ R.drawable.camera_flash_open, R.drawable.camera_flash_close};
    public static final int[] RES_DRAWABLE_CAMERA_DIRECTION = { R.drawable.camera_icon_lenses_autodyne,R.drawable.camera_icon_lenses};

    public static final String[] RES_STRING_FLASHLIGHT = {"开启","关闭"/*R.string.topic_camera_flashlight_auto, R.string.topic_camera_flashlight_on, R.string.topic_camera_flashlight_off*/};
    public static final String[] RES_STRING_CAMERA_DIRECTION = {"自拍", "帮拍"};

    public static final int LEN_PIC = DensityUtil.dip2px(35);   //图片的边长   px
    ...
    private CameraManager(Context context) {
        ...
        mLightStatus = FlashLigthStatus.valueOf(SPUtils.getIntData(SP_LIGHT_STATUE, FlashLigthStatus.LIGHT_ON.ordinal())); //默认 自动
        mFlashDirection = CameraDirection.valueOf(SPUtils.getIntData(SP_CAMERA_DIRECTION, CameraDirection.CAMERA_BACK.ordinal())); //默认后置摄像头
    }
    ...
    public void setLightStatus(FlashLigthStatus mLightStatus) {
        ...
        if (m_tvFlashLight != null) {
            ...
            m_tvFlashLight.setCompoundDrawables( null, drawable,null, null);
        }
    }
    ...
    public void setCameraDirection(CameraDirection mFlashDirection) {
        ...
        if (m_tvCameraDireation != null) {
            ...
            drawable.setBounds(0, 0, LEN_PIC, LEN_PIC);
            m_tvCameraDireation.setCompoundDrawables( null,drawable, null, null);
            //不再
...
//        }
        /*if(m_tvCameraDireation != null){
            m_tvCameraDireation.setVisibility(isFrontTakePhoto ? View.VISIBLE : View.INVISIBLE);

            m_tvCameraDireation.setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View v) {
                    setCameraDirection(getCameraDirection().next());
    ...
    private Camera.Size findFitPreResolution(Camera camera) throws Exception {
        ...
        Camera.Size resultSize = null;
        float rate =  BaseApp.getHeight()/(float)BaseApp.getWidth();
        for (Camera.Size size : supportedPicResolutions) {
            if(size.width <= ALLOW_PIC_LEN){
                if(resultSize == null){
                    resultSize = size;
                }else{
                    if((Math.abs(size.width/(float)size.height - rate)) <= (Math.abs(resultSize.width/(float)resultSize.height - rate))){
                        resultSize = size;
                    }
                }
            }
    ...
    public void bindOptionMenuView(TextView mFlashLight, TextView tvDirection, SquareCameraContainer mCameraContainer) {
        this.mCameraContainer = mCameraContainer;
        bindOptionMenuView(mFlashLight,tvDirection);
    }
