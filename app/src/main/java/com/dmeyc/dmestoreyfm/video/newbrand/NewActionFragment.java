...
public class NewActionFragment extends BaseFragment  {
    ...
    private NoScrollViewPager actionview_pager;
...
//    private ImageView actionadd_channel_iv;
    ...
    public NewActionFragment(TextView cityname,String title){
        mTitle = title;
    ...
    protected int getLayoutRes() {
        return R.layout.item_rv_new_action;
    }
    @Override
    protected void initData() {
        ...
        iv_left_title_bar = (ImageView)mRootView.findViewById(R.id.iv_left_title_bar);
        iv_left_title_bar.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                getActivity().finish();
            }
        });
        tv_Show_title = (TextView)mRootView.findViewById(R.id.mTitle);
...
//        tv_type.setOnClickListener(this);
        ...
        actionview_pager = (NoScrollViewPager) mRootView.findViewById(R.id.actionview_pager); //使用bind 会出现空指针
...
//                    tv_threenextdaybello.setText(now.get(Calendar.MONTH)+4+"月");
    ...
    private void setCurrentPage(){
        ...
        for(int i = 0 ; i < mTitleLists.size() ; i++){
            if(mTitle.equals(mTitleLists.get(i))){
                count = i;
            }
        }
        actionview_pager.setCurrentItem(count);
    }

    protected void getTitleList(List<String> mTitleLists) {
          mTitleLists.add("羽毛球"); //0
          mTitleLists.add("运动教社");//1
          mTitleLists.add("合作商户");//2
//          mTitleLists.add("校园");
//          mTitleLists.add("其他");
    }
