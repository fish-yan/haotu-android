package com.codbking.widgets;

import android.content.Context;
import android.widget.TextView;

import com.codbking.widget.R;
import com.codbking.widgets.view.WheelView;
...
class DatePicker extends BaseWheelPick {
    ...
    private Integer[] yearArr, mothArr, dayArr, hourArr;
...
//        minuteView.setCurrentItem(datePicker.findIndextByValue(datePicker.getToady(DatePickerHelper.Type.MINUTE), minutArr));
    ...
    private void setChangeDaySelect(int year, int moth) {
        ...
        }  else if(1==moth||3==moth||5==moth||7==moth||8==moth||10==moth||12==moth){
            dayView.setCurrentItem(indxt);
        }else if(4==moth||6==moth||11==moth||9==moth){
            dayView.setCurrentItem(indxt);
        }else {
            dayView.setCurrentItem(indxt);
        }
    ...
    public void onChanged(WheelView wheel, int oldValue, int newValue) {
        ...
        int day=0;
        if(1==moth||3==moth||5==moth||7==moth||8==moth||10==moth||12==moth){
            day  = dayArr[dayView.getCurrentItem()];
        }else if(4==moth||6==moth||11==moth||9==moth){
            if(30==dayView.getCurrentItem()){
              day = dayArr[29];
            }else {
              day = dayArr[dayView.getCurrentItem()];
            }
...
//        int minut = minutArr[minuteView.getCurrentItem()];
        ...
        if (onChangeLisener != null) {
            onChangeLisener.onChanged(DateUtils.getDate(year, moth, day, hour));
...
//        return DateUtils.getDate(year, moth, day, hour, minut);
        return DateUtils.getDate(year, moth, day, hour);
    }
